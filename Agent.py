...
class AStarNode(Node):
    ...
    def heuristic_function(self):
        # TODO here
        if AStarNode.END_COORDS is None:
            raise ValueError("END_COORDS not set")

        distances = []

        # multiple goals
        # calculate the Euclidean distance between the current coordinate and the end coordinate
        for coord in AStarNode.END_COORDS:
            x1, y1 = self.coord
            x2, y2 = coord
    ...
    def __eq__(self, other):
        # TODO here
        return self.path_cost + self.h_val == other.path_cost + self.h_val
...
class Agent:
    ...
    def goal_test(self, node):
        ...
        return node.coord in self.maze.end  # Check if the node's coordinate is in the set of end coordinates
    ...
    def expand_node(self, node):
        ...
        for action in self.maze.valid_ordered_action(s):
            ...
            new_cost = node.path_cost + self.maze.action_cost(action) #NOT SURE
    ...
    def bfs(self):
        ...
        reached = {}
        reached[start_node.coord] = 0

        # if the start node is the goal
        if self.goal_test(start_node):
            path = start_node.trace_back()
            return True, path, self.expansion_history
        
        # if the start node is not the goal, pop
        while not frontier.empty():
            current_node = frontier.get() # retrieve and move the first node in the queue

            # expand the node
            for child_node in self.expand_node(current_node):
                # goal test
                if self.goal_test(child_node):
                    path = child_node.trace_back()
                    return True, path, self.expansion_history
                if child_node.coord not in reached or child_node.path_cost < reached[child_node.coord]:
                    reached[child_node.coord] = child_node.path_cost
    ...
    def dfs(self):
        ...
        while not frontier.empty():
            ...
            # if fails the goal test, then expand 
            for child_node in self.expand_node(current_node):
                if child_node.coord not in reached or child_node.path_cost < reached[child_node.coord]:
                    reached[child_node.coord] = child_node.path_cost
                    frontier.put(child_node)
    ...
    def astar(self):
        self.clear_expansion_history()
        # TODO : Your Implementation :)
        # set the end coordinates for the A* heuristic
        ...
        while not frontier.empty():
            ...
            # expand the node
            for child_node in self.expand_node(current_node):
                if child_node.coord not in reached or child_node.path_cost < reached[child_node.coord].path_cost:
                    reached[child_node.coord] = child_node
                    frontier.put(child_node)
